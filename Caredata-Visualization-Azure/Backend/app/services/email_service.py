...
def send_password_reset_email(to_email: str, token: str, first_name: str = ""):
    ...
    subject = "Reset your CareData Portal password"
    html_body = f"""
    <div style="font-family: Inter, Arial, sans-serif; max-width: 520px; margin: 0 auto; padding: 32px;">
      <div style="text-align: center; margin-bottom: 24px;">
        <h1 style="color: #111; font-size: 22px; margin: 0;">CareData Portal</h1>
      ...
      <p style="color: #888; font-size: 12px; line-height: 1.5;">
        Or copy and paste this link into your browser:<br/>
        <a href="{reset_url}" style="color: #ff7b00; word-break: break-all;">{reset_url}</a>
      </p>
    ...
    if _smtp_configured():
        try:
            ...
            _log_reset_to_console(to_email, reset_url)
    else:
        _log_reset_to_console(to_email, reset_url)


def _log_reset_to_console(to_email: str, reset_url: str):
    logger.info("PASSWORD RESET (dev mode) for %s: %s", to_email, reset_url)
